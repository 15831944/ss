...
QC_ApplicationWindow::QC_ApplicationWindow()
    ...
    , partWidget(new ss_PartWidget(QObject::tr("Part Factory"),this))
    , m_dlg_showtest(new test_dia(this))
{
...
*/
    ...
    setStyleSheet("QC_ApplicationWindow#main_window{background-color:#303030;border:0px;color:white}");
    RS_DEBUG->print("QC_ApplicationWindow::QC_ApplicationWindow: init status bar");

    QStatusBar* status_bar = statusBar();
//    coordinateWidget = new QG_CoordinateWidget(this->widget_factory->fileWindow1, "coordinates");
...
//    status_bar->addWidget(selectionWidget);
    ...
    enable_left_sidebar = settings.value("EnableLeftSidebar", 1).toBool();
...
//    LC_WidgetFactory widget_factory(this, a_map, ag_manager);
      widget_factory = new LC_WidgetFactory(this, a_map, ag_manager);
...
//layerWidget->setStyleSheet("background-color:#4d4d4d");
...
//mdiAreaCAD->setStyleSheet("background-color:#4d4d4d;");
    ...
    widget_factory->slotClose(true);
    /*修改*/
    this->setWindowFlags(Qt::FramelessWindowHint);
    pTitleBar = new TitleBar(this);
    this->installEventFilter(pTitleBar);
    pTitleBar->setStyleSheet("font-size:16px;font-weight:bold");
    this->setWindowTitle("SIASUN SiaCAD 2021");
    //pTitleBar->hide();
    ...
    connect(mHelper, &FramelessHelper::sigMax, this, &QC_ApplicationWindow::slotMax);
    connect(pTitleBar->m_pCloseButton,SIGNAL(clicked()),this,SLOT(on_btnExit_clicked()));
    connect(pTitleBar->m_pMinimizeButton,SIGNAL(clicked()),this,SLOT(on_btnMin_clicked()));
    connect(pTitleBar->m_pMaximizeButton,SIGNAL(clicked()),this,SLOT(on_btnMax_clicked()));
//    mHelper->setRubberBandOnResize(true);
    this->setWindowFlags(windowFlags() | Qt::WindowMinimizeButtonHint);
    coordinateWidget = new QG_CoordinateWidget(this->widget_factory->fileWindow1, "coordinates");
    coordinateWidget->setStyleSheet("color:white;");
    selectionWidget = new QG_SelectionWidget(this->widget_factory->fileWindow1, "selections");
    selectionWidget->setStyleSheet("color:white;");
...
void QC_ApplicationWindow::slotCloseWindows()
{
    widget_factory->slotClose(true);
}
...
void QC_ApplicationWindow::enableFileActions(QC_MDIWindow* w)
{
	if (!w || w->getDocument()->getFilename().isEmpty()) {
        a_map["FileSave"]->setText(tr("保     存"));
        a_map["FileSaveAs"]->setText(tr("另     存"));
	}
	else {
...
void QC_ApplicationWindow::slotUpdateActiveLayer()
{
    ...
    {
        m_pActiveLayerName->activeLayerChanged(layerWidget->getActiveName());//修改
        selectionWidget->activeLayerChanged(layerWidget->getActiveName());
    }
}
...
QC_MDIWindow* QC_ApplicationWindow::slotFileNew(RS_Document* doc) {
    ...
    w->setObjectName("w");
    w->setStyleSheet("QWidget#w{background-color:#707070;border:0px;margin:0}");
    w->parentWidget()->setStyleSheet("border:0;margin:0;padding:0");
//    w->setStyleSheet("background-color:#707070;border-width:0px;margin:0");
    ...
    QG_DIALOGFACTORY->setMouseWidget(snapToolBar->mouseWidget);
    QG_DIALOGFACTORY->setCoordinateWidget(coordinateWidget);//修改
...
bool QC_ApplicationWindow::slotFileNewHelper(QString fileName, QC_MDIWindow* w) {
    ...
    RS_DIALOGFACTORY->updateCoordinateWidget(RS_Vector(0.0,0.0),      //修改
...
//导入零件时的槽函数
void QC_ApplicationWindow::slotPartFileOpen() {                  //读取dxf格式文件
    ...
    f->setNameFilter("支持文件(*.dxf *.DXF)");
    f->setLabelText(QFileDialog::LookIn, u8"文件目录：");
    f->setLabelText(QFileDialog::FileName, u8"文件名字：");
    f->setLabelText(QFileDialog::FileType, u8"文件类型：");
    f->setLabelText(QFileDialog::Accept, u8"打开");
    f->setLabelText(QFileDialog::Reject, u8"取消");

    f->setOption(QFileDialog::DontUseNativeDialog, true);

    connect(f,SIGNAL(currentChanged(QString)),this,SLOT(slotfilesSelected(QString)));
    layout = static_cast<QGridLayout*>(f->layout());
    ...
    layout->addWidget(View, 1, 15, 10, 50);
}

void QC_ApplicationWindow::slotfilesSelected(QString path)
{
//    RS_FilterDXFRW* fileDXF = new RS_FilterDXFRW();
//    //RS_Graphic graphic;
//    fileDXF->fileImport((RS_Graphic)scene,path,RS2::FormatType::FormatDXF1);
//    qDebug() << "执行之后";
...
void QC_ApplicationWindow::showAboutWindow()//about
{
    // author: ravas
...
//    // Src/qtbase/src/corelib/global/qcompilerdetection.h
    ...
    about aa;
    aa.setLabelText(info);
//    connect(aa.button1, SIGNAL(released()), aa, SLOT(accept()));
    aa.exec();
...
void QC_ApplicationWindow::invokeLinkList()//xxxx
{
    // author: ravas
...
void QC_ApplicationWindow::updateGridStatus(const QString & status)
{
   ...
   snapToolBar->grid_status->setBottomLabel(status);
}
...
void QC_ApplicationWindow::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
...
void QC_ApplicationWindow::invokeLicenseWindow()//license
{
    // author: ravas
License license;
license.exec();
...
//测试窗口槽函数
void QC_ApplicationWindow::showtest()
{
    qDebug()<<"Do Data Process";

    m_dlg_showtest->setModal(false);
    m_dlg_showtest->show();
    showArrangement->setModal(false);
     showArrangement->show();
    qDebug()<<"Process Finish";
}

void QC_ApplicationWindow::on_btnMin_clicked()
{
    showMinimized();
}

void QC_ApplicationWindow::on_btnMax_clicked()
{
    mHelper->setMax(this, !mHelper->isMax(this));
}

void QC_ApplicationWindow::on_btnExit_clicked()
{
    close();
}

void QC_ApplicationWindow::slotMax(bool bMax)
{
    if (bMax)
    {
        pTitleBar->m_pMaximizeButton->setStyleSheet("QPushButton{background:url(':/main/scale.png')no-repeat center center;background-color:transparent;border:0px;margin-top:1px}"\
                                                         "QPushButton:pressed{background-color:rgb(202,202,203)}"\
                                                         "QPushButton:hover{background-color:rgb(229,229,229)}");
//        pTitleBar->m_pMaximizeButton->setIcon(QIcon(":/main/scale.png"));
    }
    else
    {
//        pTitleBar->m_pMaximizeButton->setIcon(QIcon(":/main/max.png"));
                    pTitleBar->m_pMaximizeButton->setStyleSheet("QPushButton{background:url(':/main/max.png')no-repeat center center;background-color:transparent;border:0px;margin-top:1px}"\
                                                         "QPushButton:pressed{background-color:rgb(202,202,203)}"\
                                                         "QPushButton:hover{background-color:rgb(229,229,229)}");
    }
}
...
void QC_ApplicationWindow::ptrefresh()
{
    ...
    //刷新
    RS_GraphicView* reFresh = this->getGraphicView();
